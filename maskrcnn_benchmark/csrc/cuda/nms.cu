...
at::Tensor nms_cuda(const at::Tensor boxes, float nms_overlap_thresh) {
  ...
  const auto size_allocation = boxes_num * col_blocks * sizeof(unsigned long long);
  char* scratch_space = allocator.allocate(size_allocation);
  auto mask_dev = (unsigned long long*) scratch_space;
  ...
  allocator.deallocate(scratch_space, size_allocation);
